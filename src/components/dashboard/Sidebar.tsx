...
import { HomeIcon, PlusIcon, BookIcon, BookUserIcon, SidebarClose, SidebarOpenIcon, IdCardIcon, UserIcon, Building, FlowerIcon, Settings } from 'lucide-react';
...
const Item = ({ title, to, icon: Icon, children, selected, setSelected, isCollapsed }: {
    title: string,
    to: string,
        ...
        <TooltipProvider>
            <Tooltip>
                <TooltipTrigger asChild>
                    <Link
                        href={to}
                        className={`flex h-9 w-9 items-center justify-center rounded-lg hover:scale-110 duration-75 ${selected === title ? 'bg-white text-blue-700 border-gray-500' : 'text-gray-300 border-gray-300'} border-2 transition-colors`}
                        prefetch={false}
                        onClick={() => { setSelected(title); }}
                    >
                        <div className="flex items-center justify-center">
                            {/* Render the icon as JSX */}
                            {Icon && <Icon />} {/* Ensure the icon is rendered as a component */}
                        </div>
                    </Link>
                </TooltipTrigger>
...
const SideBar = () => {
    ...
    const filteredLinks = sidebarLinks.filter(link => {
        ...
        if (link.children) {
            link.children = link.children.filter(child => hasPermission(child.role));
        }
    ...
    return (
        <div className={` ${isCollapsed ? "w-20" : "w-64"} p-2 pl-0 flex flex-col gap-4 transition-all duration-100 ease-linear bg-blue-400`}>
            {/* Sidebar Toggle */}
            <div className="flex justify-between items-center cursor-pointer p-2">
                <p className={`text-lg font-bold ml-2 ${isCollapsed ? 'hidden' : 'inline'}`}>DASHBOARD</p>
                <div
